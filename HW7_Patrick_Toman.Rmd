...
Following the notation of @Marin we first define the following 
...
In the same spirit as @Marin we have
...
An implementation of the gibbs sampler is given below.
...
sample_lambda <- function(z,k){
    ...
    n_j <- colSums(outer(z,1:k,"=="))
    
    lambda <- gtools::rdirichlet(1,n_j+1)
...
gibbs_sampler <- function(x,k,niter,prior_list = list(mean=0,prec=0.1,shape=0.5,scale=10)){
...
gibbs_sampler_test <- gibbs_sampler(x=x,k=k_set,niter=niter_set)
...
Shown below are histograms of the posterior densities of our unknown parameters.

### Posterior Densities for Assignmentss
